...
let
  cfg = config.apps.daily-containers;
  uid = builtins.toString config.users.users.${username}.uid;
in {
  options.apps.daily-containers = {
    enable = lib.mkEnableOption "daily GUI application containers";
    debian.enable = lib.mkEnableOption "Debian daily container" // { default = true; };
    kali.enable = lib.mkEnableOption "Kali daily container" // { default = false; };
  };

  config = lib.mkIf (config.apps.containers && cfg.enable) (lib.mkMerge [
    (lib.mkIf cfg.debian.enable (let
      debianContext = pkgs.runCommand "daily-debian-context" {} ''
        mkdir $out
        cp ${./debian/Dockerfile} $out/Dockerfile
      '';
